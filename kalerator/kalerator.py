    ...
    def generate(self):
        ...
        for row in self.keys:
            for key in row:
                ...
                diode_pos = [key['coord'][i] + self.layout_diode_offset[i] for i in range(2)]
                diode_neg_pos = [key['coord'][i] + self.layout_diode_neg[i] for i in range(2)]
                diode_pos_pos = [key['coord'][i] + self.layout_diode_pos[i] for i in range(2)]
                sw_pin_0_pos = [key['coord'][i] + self.layout_sw_pin_0[i] for i in range(2)]

                board_scr.append('ROTATE R180 %s;' % (key['label']))
                board_scr.append('MOVE %s (%s %s);' % (key['label'], float_to_str(key['coord'][0]), float_to_str(key['coord'][1])))
                board_scr.append('ROTATE R270 D%s;' % (key['label']))
                board_scr.append('MOVE D%s (%s %s);' % (key['label'], float_to_str(diode_pos[0]), float_to_str(diode_pos[1])))
                board_scr.append('WIRE 16 %s (%s %s) (%s %s);' % (self.trace_width, diode_neg_pos[0], diode_neg_pos[1], float_to_str(sw_pin_0_pos[0]), float_to_str(sw_pin_0_pos[1])))
                ...
                if last_key_pos:
                    ...
                    left_net_pos = [last_switch_position[i] + self.sch_net_offset[i] for i in range(2)]
                    right_net_pos = [switch_position[i] + self.sch_net_offset[i] for i in range(2)]
                    ...
                    if left_net_pos[1] == right_net_pos[1]:
                        ...
                        left_diode_pos_pos = [last_key_pos[i] + self.layout_diode_pos[i] for i in range(2)]
                        board_scr.append('WIRE 16 %s (%s %s) (%s %s);' % (self.trace_width, float_to_str(left_diode_pos_pos[0]), float_to_str(left_diode_pos_pos[1]), float_to_str(diode_pos_pos[0]), float_to_str(diode_pos_pos[1])))
        ...
        for column in range(1, maxcol):
            ...
            for row in self.keys:
                ...
                if last_key_pos:
                    ...
                    top_pin_pos = (top_pin_pos[0] + self.sch_col_offset[0], top_pin_pos[1] + self.sch_col_offset[1])
                    bot_pin_pos = (bot_pin_pos[0] + self.sch_col_offset[0], bot_pin_pos[1] + self.sch_col_offset[1])
        ...
        for row in self.rawdata:
            ...
            for item in row:
                if isinstance(item, (str, unicode)):
                    ...
                    current_attributes['next_key'] = self.default_next_key.copy()
